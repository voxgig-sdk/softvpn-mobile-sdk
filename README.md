...
SoftVPN Mobile API client, generated from the OpenAPI spec.

This API provides access to the SoftVPN mobile application, offering functionalities such as retrieving a list of VPN servers.

Learn more about SoftVPN Mobile API at [github.com/l0v3m0n3y/softvpn](https://github.com/l0v3m0n3y/softvpn).

This is an unofficial SDK for the SoftVPN Mobile public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — RequestInfo and Server — that you
...
```go
client := sdk.Test()
result, err := client.RequestInfo(nil).Load(
    nil, nil,
)
```
...
// Load requestinfo data (returns a RequestInfo)
const requestinfo = await client.RequestInfo().load()
console.log(requestinfo)
```
...
```bash
cd go-mcp && go build -o softvpn-mobile-mcp .
```
...
```json
{
  "mcpServers": {
    "softvpn-mobile": {
      "command": "/abs/path/to/softvpn-mobile-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **RequestInfo** | The RequestInfo entity (load). | `/request-ip` |
| **Server** | The Server entity (list). | `/servers` |
...
# Load a specific requestinfo (returns the record, raises on error)
requestinfo = client.RequestInfo().load()
print(requestinfo)
```
...
```go
import sdk "github.com/voxgig-sdk/softvpn-mobile-sdk/go"

client := sdk.New()

// Load requestinfo data
...
Generated from the SoftVPN Mobile API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
