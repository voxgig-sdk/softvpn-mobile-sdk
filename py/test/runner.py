...
class SoftvpnMobileTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SoftvpnMobileTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SoftvpnMobileTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SoftvpnMobileTestRunner.getenv("SOFTVPN_MOBILE_TEST_LIVE")
        override = SoftvpnMobileTestRunner.getenv("SOFTVPN_MOBILE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SoftvpnMobileTestRunner.getenv(key)
        ...
        explain = SoftvpnMobileTestRunner.getenv("SOFTVPN_MOBILE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SOFTVPN_MOBILE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SoftvpnMobileTestRunner._test_control is not None:
            return SoftvpnMobileTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SoftvpnMobileTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SoftvpnMobileTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SoftvpnMobileTestRunner.load_test_control()
...
def load_env_local():
    SoftvpnMobileTestRunner.load_env_local()


def env_override(m):
    return SoftvpnMobileTestRunner.env_override(m)


def entity_data(v):
    return SoftvpnMobileTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SoftvpnMobileTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SoftvpnMobileTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SoftvpnMobileTestRunner.load_test_control()


def live_client_options():
    return SoftvpnMobileTestRunner.live_client_options()


def live_delay_ms():
    return SoftvpnMobileTestRunner.live_delay_ms()
