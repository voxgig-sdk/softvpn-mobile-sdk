...
from softvpnmobile_sdk.utility import register

# Load features
from softvpnmobile_sdk.feature.base_feature import SoftvpnMobileBaseFeature
from softvpnmobile_sdk.features import _has_feature, _make_feature
...
class SoftvpnMobileSDK:
    ...
    def __init__(self, options=None):
        ...
        utility = SoftvpnMobileUtility()
        self._utility = utility

        from softvpnmobile_sdk.config import shared_config
    ...
    def get_utility(self):
        return SoftvpnMobileUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("SoftvpnMobileSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def RequestInfo(self, data=None) -> "RequestInfoEntity":
        """Entity factory: client.RequestInfo().list() / client.RequestInfo().load({"id": ...})."""
        from softvpnmobile_sdk.entity.request_info_entity import RequestInfoEntity
        return RequestInfoEntity(self, data)


    def Server(self, data=None) -> "ServerEntity":
        """Entity factory: client.Server().list() / client.Server().load({"id": ...})."""
        from softvpnmobile_sdk.entity.server_entity import ServerEntity
        return ServerEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "SoftvpnMobileSDK":
        if sdkopts is None:
            sdkopts = {}
