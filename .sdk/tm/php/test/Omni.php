...
//    only - a typed SoftvpnMobileContext there would make every ctx
...
final class SoftvpnMobileOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SoftvpnMobileContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new SoftvpnMobileSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new SoftvpnMobileResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new SoftvpnMobileError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new SoftvpnMobileResponse($clean['response']);
    ...
    public static function err_from_map($m): ?SoftvpnMobileError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SoftvpnMobileError(is_string($code) ? $code : '', $msg);
    }
