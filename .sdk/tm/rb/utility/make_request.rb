...
module SoftvpnMobileUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SoftvpnMobileResponse.new({})
    ...
    if fetch_err
      ...
      response = SoftvpnMobileResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SoftvpnMobileResponse.new(fetched)
    else
