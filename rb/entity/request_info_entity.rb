...
class RequestInfoEntity
  ...
  def make
    opts = @_entopts.dup
    RequestInfoEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SoftvpnMobileHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [RequestInfo, Hash] the current RequestInfo data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SoftvpnMobileHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of RequestInfo fields)
  def match_get
  ...
  #
  # @param reqmatch [RequestInfoLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.RequestInfo.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [RequestInfo, Hash] the loaded RequestInfo; raises SoftvpnMobileError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SoftvpnMobileHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
