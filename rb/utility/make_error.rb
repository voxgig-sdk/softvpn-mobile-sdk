...
module SoftvpnMobileUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = SoftvpnMobileContext.new({}, nil)
    end
    op = ctx.op || SoftvpnMobileOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || SoftvpnMobileResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(SoftvpnMobileError) ? err.msg : err.to_s
    ...
    sdk_err = SoftvpnMobileError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(SoftvpnMobileError)
