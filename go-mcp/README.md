...
[MCP](https://modelcontextprotocol.io) server exposing the SoftvpnMobile SDK as
two agent tools — `softvpn-mobile_list` and `softvpn-mobile_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/softvpn-mobile-mcp)
make build

# 2. Provide credentials via the environment
export SOFTVPN_MOBILE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user softvpn-mobile \
  -- /absolute/path/to/softvpn-mobile-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./softvpn-mobile-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// softvpn-mobile_list: first page of records
{ "entity": "server" }
{ "entity": "server", "query": { } }

// softvpn-mobile_load: one record by id
{ "entity": "request_info", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/softvpn-mobile-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user softvpn-mobile \
     -- "$PWD"/dist/*/softvpn-mobile-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `softvpn-mobile_list` and `softvpn-mobile_load` tools now appear
   in new sessions. Ask the agent to *"list server using softvpn-mobile"*
   and it calls `softvpn-mobile_list` with `{"entity":"server"}`.
...
```sh
export SOFTVPN_MOBILE_APIKEY=sk_live_xxx            # API key
export SOFTVPN_MOBILE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `softvpn-mobile_list` tool
...
### Call the `softvpn-mobile_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `softvpn-mobile_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `softvpn-mobile_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SOFTVPN_MOBILE_APIKEY` | API key sent with every request. |
| `SOFTVPN_MOBILE_BASE` | Optional override of the API base URL. |
...
```sh
./softvpn-mobile-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"softvpn-mobile_load","arguments":{"entity":"request_info","query":{"id":1}}}}'
```
