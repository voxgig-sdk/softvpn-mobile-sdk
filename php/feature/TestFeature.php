...
// SoftvpnMobile SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class SoftvpnMobileTestFeature extends SoftvpnMobileBaseFeature
{
    ...
    public function init(SoftvpnMobileContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (SoftvpnMobileContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = SoftvpnMobileParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (SoftvpnMobileContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(SoftvpnMobileContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = SoftvpnMobileParam::call($ctx, $k);
