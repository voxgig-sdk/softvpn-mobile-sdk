...
// SoftvpnMobile SDK retry feature
...
class SoftvpnMobileRetryFeature extends SoftvpnMobileBaseFeature
{
    ...
    public function init(SoftvpnMobileContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SoftvpnMobileContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SoftvpnMobileContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SoftvpnMobileContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
