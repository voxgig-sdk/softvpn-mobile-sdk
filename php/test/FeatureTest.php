...
// SoftvpnMobile SDK feature test
//
...
class FtCtrl extends SoftvpnMobileControl
{
...
class FtClient
{
    ...
    private SoftvpnMobileUtility $utility;

    public function __construct(SoftvpnMobileUtility $utility, array $options)
    {
    ...
    public function get_utility(): SoftvpnMobileUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SoftvpnMobileContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SoftvpnMobileContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SoftvpnMobileContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SoftvpnMobileConfig::make_feature($name);
    ...
    private static function buildUrl(SoftvpnMobileSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SoftvpnMobileError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SoftvpnMobileResponse($fetched);
        }
    ...
    private function fail(SoftvpnMobileContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SoftvpnMobileContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
