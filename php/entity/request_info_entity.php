...
// SoftvpnMobile SDK RequestInfo entity
...
class RequestInfoEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RequestInfoEntity($this->_client, $opts);
    }

    /**
     * @param RequestInfo|array $args RequestInfo data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = SoftvpnMobileHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return RequestInfo|array The current RequestInfo data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of RequestInfo fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = SoftvpnMobileHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of RequestInfo fields).
     */
    ...
    /**
     * Load a single RequestInfo.
     *
     * @param RequestInfoLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RequestInfoLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RequestInfo|array The loaded RequestInfo as an assoc-array at the
     *   SDK boundary; throws SoftvpnMobileError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = SoftvpnMobileHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
