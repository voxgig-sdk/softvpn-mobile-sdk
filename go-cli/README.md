...
boru-driven command-line client **and** interactive REPL for the SoftvpnMobile
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/softvpn-mobile-cli)
make build

# 2. See usage (words, entities, env vars)
./softvpn-mobile-cli --help

# 3. Provide credentials once, via the environment
export SOFTVPN_MOBILE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./softvpn-mobile-cli load 1 request_info            # {id:1} shorthand
./softvpn-mobile-cli load '{id:1}' request_info       # explicit match map

# 5. Override the API base URL for a single call
SOFTVPN_MOBILE_BASE=https://api.example.com ./softvpn-mobile-cli load 1 request_info

# 6. No arguments -> interactive REPL
./softvpn-mobile-cli
softvpn-mobile> load 1 request_info
softvpn-mobile> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/softvpn-mobile-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/softvpn-mobile-cli load 1 request_info
   ```
...
```sh
./softvpn-mobile-cli load 1 request_info          # scalar shorthand for {id:1}
./softvpn-mobile-cli load '{id:1}' request_info     # explicit match map
```
...
./softvpn-mobile-cli load 1 request_info
```
...
Run with no arguments to open a REPL (prompt `softvpn-mobile>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `request_info`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/softvpn-mobile-cli`. |
...
(`./softvpn-mobile-cli <expr>`) and interactively (the REPL), and expressions compose the
