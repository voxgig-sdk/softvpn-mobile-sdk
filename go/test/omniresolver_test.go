...
import (
	...
	omni "github.com/voxgig-sdk/softvpn-mobile-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SoftvpnMobileError); is {
				out := map[string]any{
					"name":    "SoftvpnMobileError",
...
func omniCtx(arg any, client *sdk.SoftvpnMobileSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SoftvpnMobileSDK); is {
			client = live
