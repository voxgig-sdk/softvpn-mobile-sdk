...
//
// NewSoftvpnMobileSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.SoftvpnMobileSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewSoftvpnMobileSDK(opts)
}
...
func fcCandidates(client *sdk.SoftvpnMobileSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.SoftvpnMobileSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.SoftvpnMobileSDK, name string) any {
