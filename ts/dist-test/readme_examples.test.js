...
const __1 = require("..");
const SDK_NAME = 'SoftvpnMobileSDK';
...
const TEST_SEED = { "entity": { "request_info": { "test01": { "id": "test01" } }, "server": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+SoftvpnMobileSDK\b/.test(code) ||
        /\bSoftvpnMobileSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+SoftvpnMobileSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/SoftvpnMobileSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
